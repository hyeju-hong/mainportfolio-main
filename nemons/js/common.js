...
function updatePagination(swiper) {
    const curentNum = swiper.realIndex + 1; //현재 슬라이드 번호(0부터 시작이라 +1해준것)
...
visionCards.forEach((card, i) => {
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: '.vision',
            start: `top+=${i * 300} center`, //300px 마우스 휠이 내려올때마다 카드 뒤집힘
...
// vid_box  고정 처리 및 내부 비디오 스케일 애니메이션

gsap.timeline({
    scrollTrigger: {
        trigger: '.vid_box',
        start: 'top 10%',
        end: "top 10%+=1500", /* 1500px 스크롤 동안 애니메이션이 걸린다 */
        scrub: 1,
        pin: '.vid_box', /* 고정시키는 거 */
        pinSpacing: true,
        pinReparent: true,             // 부모 transform 문제 해결
    }
}).fromTo('.vid_box video', { scale: 0.65, transformOrigin: "center center" },
...
gsap.timeline({
    scrollTrigger: {
        ...
        end : ()=> "+=" + (cards.length*380), //끝나는 위치라 카드 수 *380만큼 더 내려가서 끝나게 함
...
//stagger 카드 하나씩 1초 간격으로 차례차례 등장시키는거
